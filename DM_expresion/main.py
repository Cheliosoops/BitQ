...
def autogen_run_problem(layer, qw, Nb, Nf, Nx, Ny, Nc, Nw, Nh,  kf, of, ic, parallel, numAB=[6,16],strideXY=[1,1]):
    ...
    modgen = ModGen(glb_range=global_range, tensors=[tsA, tsB, tsC],
                    ...
                    fp_coeffs={'A': 1, 'B': 1, 'C': 1},
                    cost_coeffs={'A': 1, 'B': 1, 'C': 2},
                    capacity_list=[qc*128, qc*32*1024/4, qc*256*1024/4, qc*16*1024*1024/4],   #qc(bit) i7-10700F
                    level_list=[0,1,2,4],
                    bw_list=[65, 57, 28, 16],
    ...
    with open("./files_temporary/data.txt","at") as f:
        print(cost1[0],cost1[1],cost1[2],cost1[3],lvlv[0],lvlv[1],lvlv[2],lvlv[3],Nb,Nf,Nx,Ny,Nc,Nw,Nh,kf,ic,of,numAB[0],numAB[1],strideXY[0],strideXY[1],prun,cfig,file=f)

    gen_config, select_fork = modgen.warp_sol(solution=solution_list1, fuse_tile=[['x', 'y']], erase_tile=['b', 'w', 'h'], special_scale={'L1Tc': 16})    
    with open("./files_temporary/data.py","at") as f:
