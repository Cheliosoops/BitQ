...
class ModGen:
    def __init__(self, qw, qi, qo, glb_range, tensors, idx_list, fp_coeffs, cost_coeffs, level_list, bw_list, capacity_list, parallel_list, parallelism, parallel_ids, numAB=[6,16]):
    ...
    def one_lv_cost_fp(self, cache_lv):
        ...
        if cache_lv>0:
            print('cache_lv = ',cache_lv)
            with open("./files_temporary/log.txt","at") as f:
                print('cache_lv = ',cache_lv,file=f)
    ...
    def build_all_lv_cost_fp(self):
        ...
        for cache_lv in self.level_list:
            ...
            for ts in self.tensors:
                if(ts.name == 'A'):
                    tot_fp += self.qi * fp_builder.buildFP(tensor=ts, cache_lv=cache_lv, sym_pool=self.pool, fp_coeff=self.fp_coeffs.get(ts.name))
                elif(ts.name == 'B'):
                    tot_fp += self.qw * fp_builder.buildFP(tensor=ts, cache_lv=cache_lv, sym_pool=self.pool, fp_coeff=self.fp_coeffs.get(ts.name))
                elif(ts.name == 'C'):
                    tot_fp += self.qo * fp_builder.buildFP(tensor=ts, cache_lv=cache_lv, sym_pool=self.pool, fp_coeff=self.fp_coeffs.get(ts.name))
            self.level_fp_list.append(tot_fp)
            #############################################################################################################

        print('\n##  build_all_lv_cost_fp -->')
        print('level_fp_list = ',self.level_fp_list)
        with open("./files_temporary/log.txt","at") as f:
            print('\n##  build_all_lv_cost_fp -->',file=f)
            print('level_fp_list = ',self.level_fp_list,file=f)
    ...
    def create_nest_cost_map(self):
        print('\n##  create_nest_cost_map  ##')
        with open("./files_temporary/log.txt","at") as f:
            print('\n##  create_nest_cost_map  ##',file=f)
    ...
    def create_modfile(self, nest_tuple, cost_list, target_lv, effective_lv):
        ...
        for tlv, cost_dict, bw in zip(self.level_list, cost_list, self.bw_list):
            ...
            # sum_cost_original = sum_cost                       ########################[add2] sum_cost_original
            sum_cost /= bw
            # sum_cost_original_list.append(sum_cost_original)   ########################[ad3] sum_cost_original_list
            cost_expr_list.append(sum_cost)
        #1        print('\n## create_modfile. sum_cost = ',sum_cost)
        ...
        #3        print('\n##  create_modfile = create_modfile.mod_file_name = ',mod_file_name)
        #4        print(' -> open file mod_file_name write into mod_file_name (ModGen.create_modfile 324#2)')
        ...
        # return mod_file_name, cost_expr_list,sum_cost_original_list  #######################[add4],sum_cost_original_list 
    ...
    def fix_create_run(self, nest_tuple, cost_list):
        ...
        while len(effective_lv)>0:
            ...
            for target_lv in effective_lv:
                try:
                    ...
                    # modfile_name, cost_expr_list, sum_cost_original_list = self.create_modfile(nest_tuple, cost_list, target_lv, effective_lv)  ###### [add6],sum_cost_original_list 
                    # with open("./files_temporary/log.txt","at") as f:    ############################################################################# [add7]
                        # print('sum_cost_original_list=',sum_cost_original_list,file=f)   ############################################################# [add8]

                    ampl.read(modfile_name)
                    print('\n-->  ampl read modfile_name files  <--')

                    fixed_vars = []
                    for fixtile_name in fix_dict.keys():
                        #5                        print('fixtile_name = ',fixtile_name)
                        fixvar = ampl.getVariable(name=fixtile_name)
                        #6                        print('fixvar = ',fixvar)
        ...
        print('\n##  fix_create_run -> bottleneck_list -->',bottleneck_list) 
