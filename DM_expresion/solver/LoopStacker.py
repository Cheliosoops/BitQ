...
class LoopPruner:
    def __init__(self, tensors, idx_list, cache_lv, fp_coeffs, cost_coeffs, qw, qi, qo):
    ...
    def prune_cost(self, sym_pool, erase_pmu=[], keep_pmu=[]):
        ...
        for pmu in self.all_permu:
            ...
            for ts in self.tensors:
                if(ts.name == 'A'):
                    cost_dict[ts.name] = self.qi * self.cost_coeffs.get(ts.name) * lp_stacker.build_cost_expr(
                    tensor=ts, cache_lv=self.cache_lv, sym_pool=sym_pool, fp_coeff=self.fp_coeffs.get(ts.name))
                elif(ts.name == 'B'):
                    cost_dict[ts.name] = self.qw * self.cost_coeffs.get(ts.name) * lp_stacker.build_cost_expr(
                    tensor=ts, cache_lv=self.cache_lv, sym_pool=sym_pool, fp_coeff=self.fp_coeffs.get(ts.name))
                elif(ts.name == 'C'):
                    cost_dict[ts.name] = self.qo * self.cost_coeffs.get(ts.name) * lp_stacker.build_cost_expr(
                    tensor=ts, cache_lv=self.cache_lv, sym_pool=sym_pool, fp_coeff=self.fp_coeffs.get(ts.name))    
            ###########################################################################################################
